...
def accuracy_and_or_loss_in_one_output(y_true, y_answers, returns=0):
    assert len(y_true) == len(y_answers)
...
def random_weights_for_Inicialization_and_continuation(n_neurons_list_as_array_per_layer, n=1, scale = 0.1):
    ...
    for i in range(1, len(n_neurons_list_as_array_per_layer)):
        ...
        for k in range(n_neurons_list_as_array_per_layer[i - 1]):
            a1 = random_floats(n, n_neurons_list_as_array_per_layer[i], scale)
...
def functions_of_output_2_neurons_output_layer(preço):
    '''
    Args:
        preço: qual a diferença de preço da crypto naquele estado
    Returns: um array de duas dimensões com o quanto devia comprar ou o quanto devia vender, posição0: compara, posição1:vender. Ex: [1, 0] - Comprar
    Notes: usa várias funções
    '''
    pass
...
def learning_rate_opt(time, batch_size, over_n = 0.2, longer_slope = 2):
    return over_n/(1 + math.e ** (longer_slope * ((time/(batch_size * 0.2)))))

def gradient_of_loss(true_value, predicted_value, scale = 1):
    gradient = predicted_value - true_value
    gradient = np.sign(gradient) * scale
    return gradient

def gradient_of_loss_multiple_output_neurons(true_values, predicted_values, scale = 1):
    gradient = []
    for i, j in zip(true_values, predicted_values):
        gradient.append(j - i)
    return gradient

def Softmax_0_arrays(Soft_List):
    l_r = []
    for a in range(len(Soft_List)):
        l_r.append(0)
    b = np.argmax(Soft_List)
    l_r[b] = 1
    return l_r

def accuracy_and_or_loss_in_one_output_Softmax(y_true, y_pred, epsillon = 1e-12):
    '''
    Args:
        y_true: what the output should be
        y_pred: what it really is
    Returns: the loss of the softmax activation
    Notes: does not do categorical cross entropy to facilitate computation, adapted to the needs of crypto network
    '''
    ...
    for i, j in zip(y_true, y_pred):
        true_choice = np.argmax(i)
        loss_entropy = -math.log(j[true_choice])
        loss.append(loss_entropy)
    soma = 0
    y_pred2 = []
    for i in y_pred:
        y_pred2.append(Softmax_0_arrays(i))
    for i, j in zip(y_pred2, y_true):
        if np.argmax(i) == np.argmax(j):
            soma += 1
    return sum(loss), (soma / len(y_pred))

def distance_cartesian_plain(position1, position2):
    return math.sqrt(((position2[0] - position1[0]) ** 2) + ((position2[1] - position1[1]) ** 2))

def slope(position1, position2):
    if position1[0] < position2[0]:
        position1n, position2n = position2[:], position1[:]
    else:
        position1n, position2n = position1[:], position2[:]
    return position1n[1] - position2n[1] / position1n[0] - position2n[0]

def point_slope(slope, position):
    return position[1] + (slope * position[0])

def mae_derivative(y_true, y_pred):
    """
    Calculate the derivative of Mean Absolute Error (MAE) with respect to predicted values.

    Parameters:
        y_true (float): The true target value.
        y_pred (float): The predicted output value.

    Returns:
        float: The derivative of MAE with respect to y_pred.
    """
    if y_true > y_pred:
        return -1
    elif y_true < y_pred:
...
def ReLu_derivative(inputs):
    if not(isinstance(inputs, (list, tuple))):
        return ReLu_der(inputs)
    else:
        lista_r = []
        for i in inputs:
            lista_r.append(ReLu_der(i))
        return lista_r
...
def Baralha_Listas(lista1, lista2, maximo_len=None):
    ...
    while len(lista1_baralhada) < maximo_len and len(lista2_baralhada) < maximo_len:
        random_index = random.randint(0, min(len(lista1), len(lista2)) - 1)
...
def elimina_elementos_iguais_lista_sorted(lista_r):
    lista = lista_r[:]
    ultimo_n = lista[0]
    for a, i in enumerate(lista_r[1:]):
        if i == ultimo_n:
            del lista[lista.index(ultimo_n)]
        ultimo_n = i
    return lista
