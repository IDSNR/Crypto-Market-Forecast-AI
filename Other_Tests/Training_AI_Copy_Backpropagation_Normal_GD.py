...
try:
    with open("D:\Python\Python_Crypto_AI\database_"+str(crypto_name)+"_T.json", "r") as file:
        jr = json.load(file)
...
class Layer_Dense:
    ...
    def backpropagation(self, gradient, learning_rate=0.01, alpha=0.3):
        if self.Leaky_ReLu:
            gradient_to_pass = bF.media_matrix(self.weights) * Leaky_ReLu_derivative(self.inputs, alpha) * (sum(gradient)/len(gradient))
        ...
        if not self.Leaky_ReLu:
            ...
            gradient_weights = (self.outputs[0] * gradient) * self.inputs
