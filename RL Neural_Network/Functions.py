...
def muda_pesos_RL(weights, scale):
    for a, i in enumerate(weights):
        for b, j in enumerate(i):
            weights[a][b] = perturb_float(j, scale)
    return weights

def muda_biases_RL(biases, scale):
    for a, i in enumerate(biases):
        biases[a] = perturb_float(i, scale)
    return biases
...
#pprint(random_bias_for_Inicialization_and_continuation([4, 3, 3, 2], 1, 0))
...
def gradient_of_loss(true_value, predicted_value, scale = 0.01):
    gradient = true_value - predicted_value
