...
def Leaky_ReLu(inputs, alpha=0.3):  # Leaky ReLu Activation a usar numpy
...
class Layer_Dense:
    def __init__(self, n_neurons, weights, biases, Leaky_ReLu_var=True):
        ...
        self.inputs = None
        self.outputs = None
        self.zeros_and_ones_list_biases = None
        self.zeros_and_ones_matrix_weights = None
    ...
    def backpropagation_BGD(self, gradient, learning_rate=0.01, alpha=0.3):
        if self.Leaky_ReLu:
            mean_gradient = sum(gradient)/len(gradient)
            ...
            self.weights -= learning_rate * (gradient_weights * self.zeros_and_ones_matrix_weights)
            self.biases -= learning_rate * (gradient_bias * self.zeros_and_ones_list_biases)
    ...
    def zeros_and_ones(self, probability):
        self.zeros_and_ones_list_biases = bF.get_zeros_and_ones_list(len(self.biases), probability)
        self.zeros_and_ones_matrix_weights = bF.zerosd_ones_to_matrix_zeros_ones(self.weights.shape, self.zeros_and_ones_list_biases)
...
times_batch_will_change = 40
neural_network_layers_and_its_neurons = [input_layer_n_neurons, 10, 5, 1]
iteration = 5
percentage_neurons_activated_per_batch = 0.3  #Entre 0 e 1, percentagem X 100
...
while Loss > Loss_Threshold and a15:
    ...
    for i in range(0, Batch_size):
        ...
        if (i % (Batch_size / times_batch_will_change) == 0) or (i == 0):
            for k in neural_network_layers:
                k.zeros_and_ones(percentage_neurons_activated_per_batch)
        ...
        for a21, j in enumerate(Y):
            ...
            loss_gradient = F.gradient_of_loss(expected_outputs2[a21], current_input)
