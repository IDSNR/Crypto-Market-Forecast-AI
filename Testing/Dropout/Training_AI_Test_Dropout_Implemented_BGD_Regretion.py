...
class Layer_Dense:
    ...
    def activate_dropout(self, n_neurons_took_out):
        new_bias, index_for_deleting_weights = dF.delete_n_random_elements_from_list(self.real_biases.tolist(), n_neurons_took_out)
        new_weights = dF.delete_based_on_array_of_indixes(self.real_weights, index_for_deleting_weights)

        self.weights = new_weights
        self.biases = new_bias

        return index_for_deleting_weights

    def dropout_ajustar(self, index_neurons_took_out_as_array):
        new_weights = dF.delete_matrix_2nd_dim_by_list_of_indexes(self.real_weights, index_neurons_took_out_as_array)
        self.weights = new_weights

    def backpropagation(self, gradient, learning_rate=0.01, alpha=0.3):
        gradient_to_pass = None
        if self.Leaky_ReLu:
...
iteration = 5
How_many_dropout_sessions = 10
percentage_of_neurons_taken_off = 0.1
...
while Loss > Loss_Threshold and a15:
    ...
    for i in range(0, Batch_size):
        ...
        for a21, j in enumerate(Y):
            ...
            loss_gradient.append(F.gradient_of_loss(expected_outputs2[a21], current_input))

        current_gradient = sum(loss_gradient) / len(loss_gradient)
        ...
        if (1e-2 > abs(i % (Batch_size / How_many_dropout_sessions)) and e13) or i == 1:
            e13 = False
            for l, m in reversed(zip(neural_network_layers[:-1], neural_network_layers[1:])):
                index_list = m.activate_dropout(round(m.n_neurons*percentage_of_neurons_taken_off))
                l.dropout_ajustar(index_list)

        loss_a, accuracy_a = F.accuracy_and_or_loss_in_one_output_NN(expected_outputs2, real_outputs, 0)
        ...
        if i % (Batch_size / 10) == 0:
            e13 = True
            for k in neural_network_layers:
                opt_w2.append(k.real_weights)
                opt_b2.append(k.real_biases)
