...
'''
with open("Database_weights_and_biases_"+str(crypto_name)+"_BGD.json", "r") as file:
   load = json.load(file)
...
while Loss > 1e-4:
    lists_for_optimization_weigths = []
    lists_for_optimization_bias = []
    loss = []
    accuracy = []
    a5 += 1
    for i in range(0, Batch_size):
        ...
        lists_for_optimization_bias.append(biases_matrix_continuation)
        lists_for_optimization_weigths.append(weigths_list_of_matrises_for_continuation)
        ...
        '''weigths_list_of_matrises_for_continuation = list(weigths_list_of_matrises_for_continuation)
        biases_matrix_continuation = list(biases_matrix_continuation)'''
        ...
        if i == Batch_size - 1:
            a = F.retorna_menor_indice_lista(loss)
            best_set_of_biases_yet = lists_for_optimization_bias[a]
            best_set_of_weights_yet = lists_for_optimization_weigths[a]
            a3 = 1
            a8 = True
        ...
        if a8:
            ...
            try:
                with open("Database_weights_and_biases_"+str(crypto_name)+"_BGD.json", "w") as file:
